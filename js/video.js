...
async function renderVideo(slideIndex = null) {
    ...
    const configuracoesVideo = videosRender
        .filter(modulo => modulo.configuracao) // Filtra objetos com a propriedade "configuracao"
        .map(modulo => modulo.configuracao);   // Extrai apenas o campo "configuracao"

    const { ativarVideo } = Object.assign({}, ...configuracoesVideo)
            ...
            videosArray.forEach(video => {
                ...
                if (_ativo) {
                    ...
                    if (ativarVideo) {
                        ...
                        const containerVideo = document.querySelector(".pagina-tipo-video--box-video")
                        containerVideo.innerHTML = `
                       <div class="container ">
                            <div class="alert alert-warning text-center d-flex flex-column align-items-center" role="alert">
                                <img src="../assets/prohibition.png" class="img-fluid w-25 w-md-25 mb-3" alt="Ícone de vídeo não disponível">
                                <h4 class="alert-heading">Oops!</h4>
                                <p>Esta unidade não possui vídeo ativo.</p>
                            </div>
                        </div>
