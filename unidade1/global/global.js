...
// Função para converter string de tempo ("1s", "0.5s", "200ms", "500") em ms (number)
function parseTimeToMs(time) {
  if (typeof time === "number") {
    // Se já for número, assumimos que é milissegundos
    return time;
  }
  if (typeof time === "string") {
    ...
    // Se terminar com "ms"
    if (t.endsWith("ms")) {
      return parseFloat(t.replace("ms", "")) || 0;
    }
    // Se terminar com "s"
    if (t.endsWith("s")) {
      let sec = parseFloat(t.replace("s", ""));
      return isNaN(sec) ? 0 : sec * 1000;
    }
    // Se for só número sem sufixo, assumimos ms
