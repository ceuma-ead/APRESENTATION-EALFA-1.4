...
class BootstrapCustomizer {
  ...
  generateColors(sectionKey, sectionValue, bootstrapConfig) {
    ...
    // Gerar classes base sem breakpoint (aplicam a todos os tamanhos por padrão)
    Object.entries(sectionValue).forEach(([colorName, colorValue]) => {
      css += `.bg-${colorName} { background-color: var(--bs-${colorName}) !important; }\n`;
      css += `.text-${colorName} { color: var(--bs-${colorName}) !important; }\n`;
      css += `.border-${colorName} { border-color: var(--bs-${colorName}) !important; }\n`;
    ...
    // Gerar classes responsivas com breakpoints em ordem inversa (desktop-first)
    if (bootstrapConfig.grid && bootstrapConfig.grid.breakpoints) {
      const breakpoints = bootstrapConfig.grid.breakpoints;

      // Ordenar breakpoints do maior para o menor (inverso da abordagem bootstrap)
      const sortedBreakpoints = Object.entries(breakpoints)
        .filter(([name]) => name !== 'xs') // Pular xs pois já é o padrão
        .sort((a, b) => parseInt(b[1]) - parseInt(a[1])); // Ordenação invertida

      // Valores para os breakpoints max-width
      ...
      // Gerar media queries em ordem decrescente (desktop-first)
      sortedBreakpoints.forEach(([breakpointName, breakpointValue]) => {
        const maxWidth = maxWidthValues[breakpointName] || breakpointValue;
        ...
        Object.entries(sectionValue).forEach(([colorName, colorValue]) => {
          ...
          css += `  .btn-${breakpointName}-${colorName} { background-color: var(--bs-${colorName}) !important; border-color: var(--bs-${colorName}) !important; }\n`;
        });
